...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        get("/movies/:username/recommendations", (request, response) -> userRecommendationGet(request));
    ...
    private static Object userFavoriteGet(Request request, Response response) {
        ...
        String query = "select m.title, m.picture_url from movie m, likes l where l.movieId = m.id and l.uname = \'" + uname + "\'";
        try {
            ...
            while (rs.next()) {
                Movie currMovie = new Movie(rs.getString("m.title"), rs.getString("m.picture_url"));
    ...
    private static String registerUser(Request request, Response response) {
        ...
        if (!userExist.isEmpty()) {
            ...
            String addUser = "INSERT INTO user values (:userName, :password, :date, 1, 1, 1, 1, 1, 1, 1)";
            try (Connection conn = sql2o.open()) {
    ...
    private static String userRecommendationGet(Request request) {
        Map<String, Object> model = new HashMap<>();
        String uname = request.session().attribute("currentUser");
        try {
